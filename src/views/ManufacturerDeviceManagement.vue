...
const fetchItems = async () => {
  loading.value = true
  try {
    const res = await manufacturerDeviceService.getAllManufacturerDevices()
    if (res.statusCode === '200' || res.statusCode === 'SUCCESS') {
      manufacturerDevices.value = res.data || []
    } else {
...
const handleSubmit = async () => {
  ...
  if (modalMode.value === 'edit') {
    try {
      const res = await manufacturerDeviceService.updateManufacturerDevice(currentItem.value.id, payload)
      if (res.statusCode === '200' || res.statusCode === 'SUCCESS') {
        alert('ManufacturerDevice updated successfully!')
        closeModal()
        fetchItems()
      } else {
  ...
  try {
    const res = await manufacturerDeviceService.createManufacturerDevice(payload)
    if (res.statusCode === '200' || res.statusCode === 'SUCCESS') {
      alert('ManufacturerDevice created successfully!')
      closeModal()
      fetchItems()
    } else {
...
const handleDelete = async (id) => {
  ...
  try {
    const res = await manufacturerDeviceService.deleteManufacturerDevice(id)
    if (res.statusCode === '200' || res.statusCode === 'SUCCESS') {
...
<template>
  <div>
    <div class="page-header">
      <h1 class="page-title">Manage ManufacturerDevice</h1>
      <button @click="openAddModal" class="btn btn-primary">Add ManufacturerDevice</button>
    </div>
    ...
    <div v-else class="card">
      <div class="table-responsive">
        <table class="table">
          ...
          <tbody>
            <tr v-if="manufacturerDevices.length === 0">
              <td colspan="3" class="empty-state">No items available.</td>
            </tr>
            <tr v-for="item in manufacturerDevices" :key="item.id">
              <td>{{ item.id }}</td>
              <td>{{ item.name }}</td>
    ...
    <div v-if="showModal" class="modal-overlay" @click.self="closeModal">
      <div class="modal-content card">
        <div class="modal-header">
          <h2>{{ modalMode === 'add' ? 'Add New ManufacturerDevice' : 'Edit ManufacturerDevice' }}</h2>
