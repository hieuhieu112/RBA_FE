...
const fetchItems = async () => {
  loading.value = true
  try {
    const res = await deviceTypeService.getAllDeviceTypes()
...
const handleSubmit = async () => {
  ...
  if (modalMode.value === 'edit') {
    try {
      const res = await deviceTypeService.updateDeviceType(currentItem.value.id, payload)
      if (res.statusCode === '200' || res.statusCode === 'SUCCESS') {
        alert('DeviceType updated successfully!')
        closeModal()
        fetchItems()
      } else {
  ...
  try {
    const res = await deviceTypeService.createDeviceType(payload)
    if (res.statusCode === '200' || res.statusCode === 'SUCCESS') {
      alert('DeviceType created successfully!')
      closeModal()
      fetchItems()
    } else {
...
const handleDelete = async (id) => {
  ...
  try {
    const res = await deviceTypeService.deleteDeviceType(id)
    if (res.statusCode === '200' || res.statusCode === 'SUCCESS') {
...
<template>
  <div>
    <div class="page-header">
      <h1 class="page-title">Manage DeviceType</h1>
      <button @click="openAddModal" class="btn btn-primary">Add DeviceType</button>
    </div>
      ...
      <div class="table-responsive">
        <table class="table">
          ...
          <tbody>
            ...
            <tr v-for="item in deviceTypes" :key="item.id">
              <td>{{ item.id }}</td>
              <td>{{ item.name }}</td>
    ...
    <div v-if="showModal" class="modal-overlay" @click.self="closeModal">
      <div class="modal-content card">
        <div class="modal-header">
          <h2>{{ modalMode === 'add' ? 'Add New DeviceType' : 'Edit DeviceType' }}</h2>
