...
const handleSubmit = async () => {
  ...
  if (modalMode.value === 'edit') {
    try {
      const res = await roleService.updateRole(currentItem.value.id, payload)
      if (res.statusCode === '200' || res.statusCode === 'SUCCESS') {
...
const handleDelete = async (id, name) => {
  if (name === 'SYSTEM_ADMIN') { alert('Cannot delete SYSTEM_ADMIN role.'); return; }
...
<template>
  <div>
    <div class="page-header">
      ...
      <button @click="openAddModal" class="btn btn-primary">Add Role</button>
    </div>
          ...
          <tbody>
            ...
            <tr v-for="item in roles" :key="item.id">
              ...
              <td class="actions-cell">
                ...
                <button v-if="item.name !== 'SYSTEM_ADMIN'" @click="handleDelete(item.id, item.name)" class="btn btn-danger btn-sm" title="Delete">
                  <Trash :size="16" />
                </button>
              </td>
            </tr>
    ...
    <div v-if="showModal" class="modal-overlay" @click.self="closeModal">
      <div class="modal-content card">
        <div class="modal-header">
          <h2>{{ modalMode === 'add' ? 'Add New Role' : 'Edit Role' }}</h2>
